 ...
 * @author ForteScarlet
 */
internal class InternalTcgMemberContainer(override val container: ConcurrentMap<String, TencentMemberImpl> = ConcurrentHashMap()) :
    ...
    inline fun computeAndGet(
        ...
        return compute(info.id.literal) { _, v ->
            v?.also {
                it.source = info
            } ?: TencentMemberImpl(bot, info, guildCalculator())
        }!!
    }
}